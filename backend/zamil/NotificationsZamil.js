...
router.get('/check-unread/:id', async (req, res) => {
    ...
    try {
        const query = `
            ...
            FROM notifications 
            WHERE zamil_id = ? AND (is_read = 0 OR is_read IS NULL OR is_read = false)
        `;
        
        const [results] = await db.promise().query(query, [zamilId]);
        ...
        console.error("❌ Error checking unread zamil notifications:", error);
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        console.error("❌ Error fetching zamil notifications:", error);
...
router.put('/:id/read', async (req, res) => {
    ...
    try {
        ...
        console.error("Error updating zamil notification status:", error);
        res.status(500).json({ success: false, message: "Server error." });
    }
});
